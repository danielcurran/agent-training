...
async function checkIndexes() {
  ...
  // Check for empty fields objects (stubs not filled in)
  const emptyIndexes = learnerIndexes.filter(i => !i.fields || Object.keys(i.fields).length === 0);
  if (emptyIndexes.length > 0) {
    console.error(`✗ ${emptyIndexes.length} index(es) have empty fields: ${emptyIndexes.map(i => i.name).join(', ')}`);
    console.error('  Fill in the fields object for each index in src/indexes.js');
    results.passed = false;
    saveResults(results);
    process.exit(1);
  }
