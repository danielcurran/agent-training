...
async function seed() {
  ...
  try {
    ...
    if (DROP) {
      ...
      if (count > 0) {
        console.log(`Collection already has ${count} documents. Use npm run seed:fresh to drop and re-seed.`);
        await client.close();
        return;
      }
    }
    ...
    const count = await collection.countDocuments();
    console.log(`\n✓ products: ${count} document(s) seeded`);
    console.log('✓ No custom indexes (learner creates these in Stage 2)');
    console.log('\nStarting state: products collection seeded, no indexes — run npm run check:env to verify.');
  } catch (err) {
