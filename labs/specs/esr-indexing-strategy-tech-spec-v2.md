# ESR Indexing Strategy Lab — Technical Specification (v2 — Controlled Test)

> **Controlled Test Variant.** This spec removes the non-ESR index contrast (Strategy B) from Stage 3 learner-active materials. Stage 1's SQL bridge (Section 1.3) and the non-ESR vs. ESR contrast in Section 1.2 are unchanged. The change is isolated to Stage 3 only.
>
> **Hypothesis under test:** Is the Stage 3 non-ESR contrast scaffolding the mechanism that enables sense-making transfer, or is the Stage 1 SQL bridge sufficient?
>
> **Expected outcome if hypothesis holds:** Learners who complete this variant still name the SORT stage signal and produce correct non-ESR contrasts at transfer, indicating they derived the contrast from Stage 1 framing alone. If sense-making transfer scores drop (△ → ✗), the Stage 3 non-ESR scaffolding is the active ingredient — not SQL bridge language.
>
> See `labs/reports/esr-indexing-strategy/esr-indexing-strategy-transfer-v1.md` Section 14 for the finding that motivated this variant.
...
The v1 spec included Strategy B (non-ESR index) as an explicit comparison in Stage 3. This variant removes it. Stage 1 still includes the SQL bridge (Section 1.3) and the non-ESR vs. ESR contrast (Section 1.2). Stage 2 still includes the before-query-3 contrast example. Stage 3 shows only before (no index) and after (ESR). The question this tests: does the learner need to see the wrong way materialised in Stage 3 — or does the Stage 1 framing carry through?
