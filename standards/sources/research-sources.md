...
- **Learning to Reason by Analogy via Retrieval-Augmented Reinforcement Fine-Tuning** (Xiao et al., arXiv 2606.13680) — Proposes RA-RFT, a post-training framework that teaches LLMs to reason by analogy; trains a retriever to rank in-context examples by expected reasoning benefit rather than semantic overlap; shows that structurally analogical examples (shared reasoning pattern, not surface similarity) outperform semantically similar ones as in-context demonstrations; directly relevant to selecting high-quality in-context examples that ground LLMs in unfamiliar problem types
...
- **SIGA: Self-Evolving Coding-Agent Adapters for Scientific Simulation** (Ho et al., arXiv 2606.09774) — Frames simulator setup as a problem of agent-tool interface grounding: what minimal adaptations are needed for an off-the-shelf coding agent to operate real scientific software? Introduces a Simulator-Interface Grounding Adapter that supplies domain-specific contracts through retrieval, procedural memory, in-trajectory validation, and validation-enforced termination; demonstrates that grounding can be reduced to a minimal, learnable adapter rather than bespoke fine-tuning
